...
import { collection, getDocs, onSnapshot, orderBy, deleteDoc, query, doc, Timestamp, addDoc, updateDoc } from "firebase/firestore";
...
function TableEncaissement(props) {

  const { currentUser, famille, encaissement, caisse, user, solde } = useContext(AuthContext);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, "encaissement", id));
    ...
    <>
      <Row>
        <div className="col">
            ...
            <Form>
                ...
                <Col md="12">
                  <FormGroup>
                    <InputGroup className="mb-4 input-group-alternative">
                      <Input id='libelle' disabled={loading} placeholder="Libellé" type="text" onChange={handleInput} />
                    </InputGroup>
                  </FormGroup>
                </Col>
              </Row>
              <Row>
                <Col md="12">
                  <FormGroup>
                    <InputGroup className="mb-4 input-group-alternative">
                      <Input id='montant' disabled={loading} placeholder="Montant en CFA" type="number" onChange={handleInput} />
                    </InputGroup>
                  </FormGroup>
                </Col>
              </Row>
          ...
          <Card className="" style={{height: 453, borderBottomRightRadius: 18, borderBottomLeftRadius: 18}}>
            ...
            <CardFooter className="py-4" style={{borderBottomRightRadius: 18, borderBottomLeftRadius: 18}}>
              <nav aria-label="...">
                <Pagination>
                  ...
                  {[...Array(totalPages)].map((_, i) => (
                    <PaginationItem key={i} active={i + 1 === currentPage}>
                      <PaginationLink onClick={() => handleClick(i + 1)} >{i + 1}</PaginationLink>
                    </PaginationItem>
                  ))}
